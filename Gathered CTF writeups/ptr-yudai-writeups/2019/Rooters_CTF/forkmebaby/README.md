# [pwn 499pts] ForkMebaby - Rooters CTF
64ビットでRELRO以外有効です。
```
$ checksec -f vuln
RELRO           STACK CANARY      NX            PIE             RPATH      RUNPATH      Symbols         FORTIFY Fortified       Fortifiable  FILE
Partial RELRO   Canary found      NX enabled    PIE enabled     No RPATH   No RUNPATH   No Symbols      Yes     0               1       vuln
```
単純なスタックオーバーフローがありますが、CanaryもPIEもASLRも有効です。しかし、forkされた子プロセスでオーバーフローできるので何回試してもCanaryやアドレスは変わりません。WinterKosenCTF 2020に出題予定だったtinytot stackというボツになった問題とほぼ同じなので割と瞬殺でした。
Canaryは1バイトずつ当てれば簡単に求まりますが、PIEが有効な上leakが無いのが厳しいです。しかし、リターンアドレスの下位1バイトを書き換えてwriteする場所に飛ばせばスタックの中身がドバーッと出るのでproc baseが求まります。
あとは順当にlibc leakしてシェルを召喚すればOKです。
```python
import threading
from ptrlib import *
logger.level = 0
