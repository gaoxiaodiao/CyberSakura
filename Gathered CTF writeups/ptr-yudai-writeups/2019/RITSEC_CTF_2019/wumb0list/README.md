# [pwn 500pts] wumb0list - RITSEC CTF 2019
64ビットバイナリで全部有効です。
```
$ checksec -f wumb0list
RELRO           STACK CANARY      NX            PIE             RPATH      RUNPATH      Symbols         FORTIFY Fortified       Fortifiable  FILE
Partial RELRO   Canary found      NX enabled    No PIE          No RPATH   No RUNPATH   No Symbols      Yes     0               3       wumb0list
```
でかバイナリ。ヒープ問っぽい。なんかどうでも良い機能が腐るほどあるけどほとんどミスリーディング用のカスなので無視。
リスト管理のインデックスにOOBがある。とりあえずlibc, heapのアドレスを抜く。
catalogの方で作ったリストはbssで管理されるのでそっちをリスト管理でfreeしてやって、catalogに戻ってもう一度freeすることでdouble freeする。あとはfree hookに繋ぐだけ。
```
from ptrlib import *
...
# 感想
バイナリでかいしstripすんなという感じだし時間制限付きdockerにssh接続しないとだめなのも意味不明。ミスリーディング多すぎで本気で面白くなかった。