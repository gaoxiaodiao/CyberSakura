# [pwn 50pts] Bronze Ropchain - RedpwnCTF 2019
32ビットでstatic linkです。Canaryが有効と書いてありますが、脆弱性のある関数では使われていません。
```
$ checksec -f bronze_ropchain
RELRO           STACK CANARY      NX            PIE             RPATH      RUNPATH      Symbols         FORTIFY Fortified       Fortifiable  FILE
Partial RELRO   Canary found      NX enabled    No PIE          No RPATH   No RUNPATH   2153 Symbols     Yes    3               44      bronze_ropchain
```
getsによるStack Overflowがあるので、ROPを作ります。
mprotectがあるのでシェルコードを呼び出せる、というのが想定解だと思います。
いつもstripされててmprotectがどこか分からないので、癖でシェルを起動するROPを組み立ててしまいました。
案外良いROP gadgetが無かったのと、payloadに改行文字が含まれたらダメなので、XOR gadgetとか使いながら回避しました。
```python
from ptrlib import *
