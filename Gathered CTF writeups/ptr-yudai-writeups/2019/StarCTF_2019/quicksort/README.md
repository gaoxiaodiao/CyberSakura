# [pwn 155pts] quicksort - *CTF 2019
64ビットバイナリですが、色々と有効になっています。
```
$ checksec -f quicksort
RELRO           STACK CANARY      NX            PIE             RPATH      RUNPATH	Symbols		FORTIFY	Fortified	Fortifiable  FILE
Partial RELRO   Canary found      NX enabled    No PIE          No RPATH   No RUNPATH   No Symbols      Yes	0		2	quicksort
```
クイックソードできるサービスです。
入力した個数分の配列をmallocで確保しているのですが、数字を入力する際にfgetsを使っているのでStack Overflowがあります。
SSPが有効なのでダメじゃんってなるのですが、mallocで確保されたアドレスを上書きできるので、結果として任意のアドレスに書き込みできます。
ということで、`__stack_chk_fail`のGOTを書き換えてSSPを無効化した後、リターンアドレスを書き換えてlibc leakし、2回目で`system('/bin/sh')`を実行します。
