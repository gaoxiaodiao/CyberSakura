# [pwn 112pts] babyheap - DEF CON CTF 2019 Qualifier
64ビットで全部有効です。
```
$ checksec -f babyheap
RELRO           STACK CANARY      NX            PIE             RPATH      RUNPATH	Symbols		FORTIFY	Fortified	Fortifiable  FILE
Full RELRO      Canary found      NX enabled    PIE enabled     No RPATH   No RUNPATH   No Symbols      Yes	1		3	babyheap
```

malloc, free, showができるサービスです。
double freeはないですが、off-by-oneがあります。
したがって、chunk overlapして`__malloc_hook`とかのアドレスをfree済みチャンクに書き込めればOKです。
しかし、freeするとmemsetで初期化したときのサイズ分消されてしまいます。
また、データを書き込む際はNULLか改行で入力を終了してしまいます。
したがって、overlapしても次のチャンクサイズを残したままfdを書き換えることができないように思えます。
しかし、よく考えるとoff-by-oneがあるので1バイトずつ書き込んでいけばできます。
```python
from ptrlib import *
...
libc-2.29はあんまり関係なかったです。

# 感想
最近libc-2.29多くなってきてない？平和に2.27使おうよ。
