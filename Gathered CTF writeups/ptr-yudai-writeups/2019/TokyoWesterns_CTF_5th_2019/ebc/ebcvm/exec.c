...
vm *exec_op(vm *_vm, inst *_inst) {
  if (_vm->regs->regs[IP] == 0x00000000040061ce || _vm->regs->regs[IP] == 0x0000000004006160 || _vm->regs->regs[IP] == 0x000000000400623e) {
    //printf("%lx <-- %lx ^ %lx\n", _vm->regs->regs[R3], _vm->regs->regs[R1], _vm->regs->regs[R6]);
  }
  if (_vm->regs->regs[IP] == 0x000000000400615c || _vm->regs->regs[IP] == 0x00000000040061ca) {
    //printf("target = %lx\n", _vm->regs->regs[R3]);
  }
  ...
  if (_vm->regs->regs[IP] == 0x0000000004006354) {
    ...
    ponta++;
    /*
    size_t addr = 0x4006000;
    _vm->regs->regs[IP] = addr;
    while (_vm->regs->regs[IP] < addr + 0xbd4) {
    ...
    */
    ...
    int i;
    for(i = 0; i < 0xbd4 / 8; i++) {
      uint64_t res = read_mem64(_vm->mem, 0x4006000 + i * 8);
      fwrite(&res, 1, 8, fp);
    }
    fclose(fp);
    exit(0);
  ...
  if (ponta == 4 && _vm->regs->regs[IP] == 0x0000000004006358) {
    _vm->regs->regs[R1] = 0x6c346e303174706f;
  }

  if (ponta == 4 && _vm->regs->regs[IP] >= 0x0000000004006354) {
    printf("%016lx    ", _vm->regs->regs[R1]);
  }
  
  dump_inst(_vm);
  /*
  if (_vm->regs->regs[IP] == 0x0000000004006172) {
    _vm->regs->regs[IP] = 0x4006354;
    size_t addr = 0x4006354;
    while (_vm->regs->regs[IP] < addr + 0x2c0 * 4) {
      size_t op_len = dump_inst(_vm);
      _vm->regs->regs[IP] += op_len;
    }
